...
export class Container2D<T extends Identifiable> implements Iterable<Container2DResult<T>>{
    ...
    set(value: T | null, x: number, y?: number | undefined): void {
        if (y === undefined) {
            const origX: number = x;

            x = origX % this.getWidth();
            y = Math.floor(origX / this.getWidth());
        }
    ...
    getRandomEmptyIndex(): number | null {
        ...
        const index: number = Math.floor(Math.random() * emptyKeys.length);
        return emptyKeys[index];
    }
